...
namespace iMoBio.PlasmidCanvas
{
    ...
    public class SimpleCircularCanvas : PlasmidCanvasUI
    {
        static public string CanvasType = "Simple Circular";
            ...
            : base()
        {
            ...
            MainStrand = LayerManager.NewLayer( Layer.LAYER_MAINSTRAND, false);
            Children.Add(MainStrand.Shape);
            ReverseStrand = LayerManager.NewLayer( Layer.LAYER_REVERSESTRAND, false);
            Children.Add(ReverseStrand.Shape);
            RulerLayer = LayerManager.NewLayer( Layer.LAYER_RULER, false, 10, 10);
            RulerLayer.Visibility = Visibility.Visible;
            Children.Add(RulerLayer.Shape); 
            RestrictSite = LayerManager.NewLayer( LayerManager.Width);
        ...
        private Cursor IbeamCursor(Point point)
        {
            ...
            if (idx >= CircularCanvas.IbeamCursors.Length || idx < 0)
            {
                idx = 0;
            }

            //Console.WriteLine("deg: {0}, idx: {1}", deg, idx);
            return CircularCanvas.IbeamCursors[idx];
        }
        #endregion
